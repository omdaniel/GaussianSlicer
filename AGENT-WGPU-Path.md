...
- **Filter Toggle Parity:** Both pipelines expose a “Filter” toggle (bilinear ↔ nearest) plus matching outline logic (gradients for bilinear, discrete band edges for nearest). WGSL stores densities in `rgba16float` so the visualize pass can use filterable sampling while preserving the legacy blocky mode behind the toggle.
